...
public class WebSecurity extends WebSecurityConfigurerAdapter {
	...
	private final BCryptPasswordEncoder bCrypPasswordEncoder;

	public WebSecurity(UserService userDetailsService, BCryptPasswordEncoder bCryptPasswordEncoder) {
		this.userDetailsService = userDetailsService;
		this.bCrypPasswordEncoder = bCryptPasswordEncoder;
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers(HttpMethod.POST, SecurityConstants.SIGN_UP_URL)
				...
				.sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS); // Tell spring not to create Http session
	}
